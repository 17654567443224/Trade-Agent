...
from ..lib.tool_cache import ToolCache, NO_UPDATE_HINT
...
def analyze_macro_impact(symbols, event_type: str = "all") -> str:
    """

    :param symbols: ETH,BTC
    :param event_type:
    :return:
    """
    md = MarketData()
    prices = md.get_prices(symbols)
    ...
    fingerprint = "|".join(n.get("title", "") for n in macro_news)
    changed = ToolCache.instance().update(f"analyze_macro_impact:{event_type}", fingerprint)
