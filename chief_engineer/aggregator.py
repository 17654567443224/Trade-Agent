...
class Aggregator:
    ...
    def __init__(self, llm_client, logger: LoggerEngine, source):
        ...
        self.logger = logger.get_logger("chief_enginner.aggregator")
        self.tools = []
        self._build_tools()
    ...
    def create_agent(self):
        with open('../config/prompts/aggregator.txt', 'r', encoding='utf-8') as f:
            sys_prompt = f.read()
        llm = self.llm_client.aggregator_model
        agent = create_agent(
            model=llm,
            tools=self.tools,
