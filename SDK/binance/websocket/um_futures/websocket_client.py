...
from SDK.binance.websocket.websocket_client import BinanceWebsocketClient
...
class UMFuturesWebsocketClient(BinanceWebsocketClient):
    ...
    def kline(self, symbol: Union[str, list], interval: Union[str, list], id=None, action=None, **kwargs):
        ...
        Update Speed: 250ms
        """
        if isinstance(symbol, list) and isinstance(interval, list):
            # 确保两个列表长度相同
            if len(symbol) != len(interval):
                # 如果 interval 只有一个，扩展到与 symbol 相同长度
                if len(interval) == 1:
                    interval = interval * len(symbol)
                else:
                    raise ValueError(f"symbol数量({len(symbol)})与interval数量({len(interval)})不匹配")

                # 构建流名称列表
            stream_name = [
                f"{sym.lower()}@kline_{intv}"
                for sym, intv in zip(symbol, interval)
            ]
        else:
